...
class SmartGenerator(settings: SmartGenerator.Settings, val accounts: Seq[PrivateKeyAccount]) extends TransactionGenerator {
  ...
  private def generate(settings: SmartGenerator.Settings): Seq[Transaction] = {
    ...
    val setScripts = Range(0, settings.scripts) flatMap (_ =>
      accounts.map { i =>
        SetScriptTransaction.selfSigned(1, i, Some(script), 1.GXB, System.currentTimeMillis()).explicitGet()
      })

    val txs = Range(0, settings.transfers).map { i =>
      TransferTransactionV2
        .selfSigned(2, None, bank, bank, 1.GXB - 2 * fee, System.currentTimeMillis(), None, fee, Array.emptyByteArray)
        .explicitGet()
    }
    ...
    val extxs = Range(0, settings.exchange).map { i =>
      ...
      val sellOrder       = OrderV2.sell(seller, matcher, pair, 100000000L, 1, ts, ts + 30.days.toMillis, 0.003.GXB)
      val buyOrder        = OrderV2.buy(buyer, matcher, pair, 100000000L, 1, ts, ts + 1.day.toMillis, 0.003.GXB)

      ExchangeTransactionV2.create(matcher, buyOrder, sellOrder, 100000000, 1, 0.003.GXB, 0.003.GXB, 0.011.GXB, ts).explicitGet()
    }
