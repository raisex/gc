          ...
          Left(
            GenericError(
              "Attempt to transfer unavailable funds: Transaction application leads to " +
                s"negative GXB balance to (at least) temporary negative state, current balance equals $oldWavesBalance, " +
    ...
    if (height >= Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs)) {
      for {
        ...
        _ <- Either.cond(
          ...
          (),
          GenericError(
            s"Fee in ${feeAssetId.fold("GXB")(_.toString)} for ${tx.builder.classTag} does not exceed minimal value of $minWaves GXB: ${tx.assetFee._2}")
        )
      } yield ()
    } else {
