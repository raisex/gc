...
object Application extends ScorexLogging {
  ...
  private def readConfig(userConfigPath: Option[String]): Config = {
    ...
    val config = maybeConfigFile match {
      ...
      case None =>
        log.warn("NO CONFIGURATION FILE WAS PROVIDED. STARTING WITH DEFAULT SETTINGS FOR TESGXBET!")
        ConfigFactory.load()
  ...
  def main(args: Array[String]): Unit = {
    ...
    System.setProperty("GXB.directory", config.getString("GXB.directory"))
    log.info("Starting...")
    sys.addShutdownHook {
