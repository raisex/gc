...
class NetworkSettingsSpecification extends FlatSpec with Matchers {
  ...
  "NetworkSpecification" should "read values from config" in {
    val config          = loadConfig(ConfigFactory.parseString("""GXB.network {
        ...
        |  }
        |}""".stripMargin))
    val networkSettings = config.as[NetworkSettings]("GXB.network")
  ...
  it should "build node name using nonce" in {
    val config          = loadConfig(ConfigFactory.parseString("GXB.network.nonce = 12345"))
    val networkSettings = config.as[NetworkSettings]("GXB.network")
  ...
  it should "fail with IllegalArgumentException on too long node name" in {
    val config = loadConfig(ConfigFactory.parseString(
      "GXB.network.node-name = очень-длинное-название-в-многобайтной-кодировке-отличной-от-однобайтной-кодировки-американского-института-стандартов"))
    intercept[IllegalArgumentException] {
      config.as[NetworkSettings]("GXB.network")
    }
  }
}
