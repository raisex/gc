...
class WavesSettingsSpecification extends FlatSpec with Matchers {
  ...
  private def config(configName: String) =
    WavesSettings.fromConfig(ConfigFactory.parseFile(new File(s"GXB-$configName.conf")).withFallback(ConfigFactory.load()))

  def testConfig(configName: String)(additionalChecks: WavesSettings => Unit = _ => ()) {
    "GXBSettings" should s"read values from default config with $configName overrides" in {
      val settings = config(configName)

      settings.directory should be(home + "/GXB")
  ...
  "GXBSettings" should "resolve folders correctly" in {
    val config = loadConfig(ConfigFactory.parseString(s"""GXB {
         |  directory = "/xxx"
