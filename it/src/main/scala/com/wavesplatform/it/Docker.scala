...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false) extends AutoCloseable with ScorexLogging {
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("GXB.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val wavesNetwork: Network = {
    val networkName = s"GXB-${hashCode().toLong.toHexString}"
  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val restApiPort    = actualConfig.getString("GXB.rest-api.port")
      val networkPort    = actualConfig.getString("GXB.network.port")
      val matcherApiPort = actualConfig.getString("GXB.matcher.port")
      ...
      val nodeName   = actualConfig.getString("GXB.network.node-name")
      ...
      val configOverrides: String = {
        val ntpServer = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-DGXB.ntp-server=$x ")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -DGXB.network.declared-address=$ip:$networkPort $ntpServer"


        if (enableProfiling) {
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2018.04/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=GXBNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot "
        }
      ...
      val containerConfig = ContainerConfig
        ...
        .env(s"GXB_OPTS=$configOverrides")
        .build()
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$GXB_OPTS.*-jar|$$GXB_OPTS $renderedConfig -jar|' $shPath && chmod +x $shPath")
...
object Docker {
  private val ContainerRoot      = Paths.get("/opt/GXB")
