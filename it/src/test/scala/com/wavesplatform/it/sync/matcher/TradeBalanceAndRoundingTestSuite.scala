...
class TradeBalanceAndRoundingTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade GXB-USD" - {
    ...
    "place usd-waves order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("GXB") shouldBe sellOrderAmount + matcherFee
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("GXB") shouldBe bobWavesBalanceBefore - (sellOrderAmount + matcherFee)
    ...
    "get opened trading markets. USD price-asset " in {
      ...
      markets.amountAssetName shouldBe "GXB"
    ...
    "check reserved balance" in {
      ...
      matcherNode.reservedBalance(bobAcc)("GXB") shouldBe expectedBobReservedBalance
    ...
    "check waves-usd tradable balance" in {
      ...
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("GXB") shouldBe expectedBobTradableBalance
      matcherNode.tradableBalance(aliceAcc, wavesUsdPair)("GXB") shouldBe aliceNode.accountBalances(aliceAcc.address)._1
      ...
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("GXB") shouldBe bobNode.accountBalances(bobAcc.address)._1
    }
  }

  "Alice and Bob trade GXB-USD check CELLING" - {
    val price2           = 289
    ...
    "place usd-waves order" in {
      // Alice wants to sell USD for Waves
      val bobWavesBalanceBefore = matcherNode.accountBalances(bobAcc.address)._1
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("GXB")
      val bobOrder1   = matcherNode.prepareOrder(bobAcc, wavesUsdPair, OrderType.SELL, sellOrderAmount2, price2)
      val bobOrder1Id = matcherNode.placeOrder(bobOrder1).message.id
      matcherNode.waitOrderStatus(wavesUsdPair, bobOrder1Id, "Accepted", 1.minute)

      matcherNode.reservedBalance(bobAcc)("GXB") shouldBe correctedSellAmount2 + matcherFee
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("GXB") shouldBe bobWavesBalanceBefore - (correctedSellAmount2 + matcherFee)
  ...
  "Alice and Bob trade WCT-USD 1" - {
    ...
    "place wct-usd order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("GXB") shouldBe expectedReservedWaves
  ...
  "Alice and Bob trade WCT-GXB on not enough fee when place order" - {
    ...
    "bob lease all  exact half matcher fee" in {
      ...
      matcherNode.tradableBalance(bobAcc, wctWavesPair)("GXB") shouldBe matcherFee / 2 + receiveAmount(SELL, wctWavesSellAmount, wctWavesPrice) - matcherFee
