...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("GXB.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("GXB.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val minerWavesBalanceAfterFirstXferTest   = minerWavesBalance + 2.GXB + minFee + CommonValidation.FeeUnit * SmallFee / minSponsorFee
    val sponsorWavesBalanceAfterFirstXferTest = sponsorWavesBalance - 2.GXB - minFee - CommonValidation.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
    ...
    "GXB fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.GXB, LargeFee, None, Some(sponsorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
      assert(!transferTxCustomFeeAlice.isEmpty)
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.GXB, TinyFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        nodes.waitForHeightArise()
        ...
        alice.assertBalances(alice.address, aliceWavesBalance._2 - 1.GXB)
        bob.assertBalances(bob.address, bobWavesBalance._1 + 1.GXB, bobWavesBalance._2 + 1.GXB)
