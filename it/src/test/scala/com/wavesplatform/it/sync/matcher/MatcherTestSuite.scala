...
class MatcherTestSuite extends MatcherSuiteBase with TableDrivenPropertyChecks {
  ...
  "Check cross ordering between Alice and Bob" - {
    ...
    val order1         = matcherNode.prepareOrder(aliceAcc, aliceWavesPair, SELL, aliceSellAmount, 2000.GXB, version = orderVersion, timeToLive = 2.minutes)
    ...
    "get opened trading markets" in {
      ...
      markets.priceAssetName shouldBe "GXB"
    ...
    "sell order could be placed correctly" - {
      "alice places sell order" in {
        ...
        orders.asks.head.price shouldBe 2000.GXB
      }
      ...
      "and should match with buy order" in {
        ...
        val order2 = matcherNode.placeOrder(bobAcc, aliceWavesPair, BUY, 200, 2.GXB * Order.PriceConstant, matcherFee, orderVersion)
      ...
      "submitting sell orders should check availability of asset" in {
        ...
        val badOrder = matcherNode.prepareOrder(bobAcc, aliceWavesPair, SELL, 300, 1900.GXB, orderVersion)
        ...
        val order3 = matcherNode.placeOrder(bobAcc, aliceWavesPair, SELL, 150, 1900.GXB, matcherFee, orderVersion)
        matcherNode.waitOrderStatus(aliceWavesPair, order3.message.id, "Accepted")

        // Bob checks that the order in the order book
        val orders = matcherNode.orderBook(aliceWavesPair)
        orders.asks should contain(LevelResponse(150, 1900.GXB))
      }
      ...
      "buy order should match on few price levels" in {
        ...
        val order4 =
          matcherNode.placeOrder(aliceAcc, aliceWavesPair, BUY, 350, (21.GXB / 10.0 * Order.PriceConstant).toLong, matcherFee, orderVersion)
      ...
      "order could be canceled and resubmitted again" in {
        ...
        val order4 = matcherNode.placeOrder(aliceAcc, aliceWavesPair, SELL, 100, 2000.GXB, matcherFee, orderVersion)
        order4.status should be("OrderAccepted")

        // Alice checks that the order is in the order book
        val orders2 = matcherNode.orderBook(aliceWavesPair)
        orders2.asks should contain(LevelResponse(100, 2000.GXB))
      }
      ...
      "buy order should execute all open orders and put remaining in order book" in {
        ...
        val order5 = matcherNode.placeOrder(bobAcc, aliceWavesPair, BUY, 130, 2000.GXB, matcherFee, orderVersion)
        ...
        val orders = matcherNode.orderBook(aliceWavesPair)
        orders.bids should contain(LevelResponse(30, 2000.GXB))
      ...
      "request order book for blacklisted pair" in {
        val f = matcherNode.matcherGetStatusCode(s"/matcher/orderbook/$ForbiddenAssetId/GXB", 404)
      ...
      "should consider UTX pool when checking the balance" in {
        ...
        def bobOrder = matcherNode.prepareOrder(bobAcc, bobWavesPair, SELL, someAssetAmount, 0.005.GXB, matcherFee, orderVersion)
        ...
        val order7 = matcherNode.placeOrder(aliceAcc, bobWavesPair, BUY, someAssetAmount, 0.005.GXB, matcherFee, orderVersion)
      ...
      "trader can buy GXB for assets with order without having GXB" in {
        val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
        ...
        def bobOrder =
          matcherNode.prepareOrder(bobAcc, bobWavesPair, SELL, someAssetAmount, 1.GXB, matcherFee, orderVersion)
