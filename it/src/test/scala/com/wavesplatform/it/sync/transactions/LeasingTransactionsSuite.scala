...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can not make leasing without having enough balance") {
    for (v <- supportedVersions) {
      ...
      assertBadRequestAndResponse(sender.lease(secondAddress, firstAddress, balance2 + 1.GXB, minFee, version = v), errorMessage)
  ...
  test("can not make leasing to yourself") {
    for (v <- supportedVersions) {
      val (balance1, eff1) = notMiner.accountBalances(firstAddress)
      assertBadRequestAndResponse(sender.lease(firstAddress, firstAddress, balance1 + 1.GXB, minFee, v), "Transaction to yourself")
      nodes.waitForHeightArise()
