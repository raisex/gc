...
class CancelOrderTestSuite extends MatcherSuiteBase {
  ...
  "Order can be canceled" - {
    "by sender" in {
      val orderId = matcherNode.placeOrder(bobAcc, wavesUsdPair, OrderType.SELL, 100.GXB, 800, matcherFee).message.id
    ...
    "works for" - {
      "all orders placed by an address" in {
        ...
        val usdOrderIds = 1 to 5 map { i =>
          matcherNode.placeOrder(bobAcc, wavesUsdPair, OrderType.SELL, 100.GXB + i, 400, matcherFee).message.id
        }

        matcherNode.assetBalance(bobAcc.toAddress.stringRepr, BtcId.base58)

        val btcOrderIds = 1 to 5 map { i =>
          matcherNode.placeOrder(bobAcc, wavesBtcPair, OrderType.BUY, 100.GXB + i, 400, matcherFee).message.id
        }
