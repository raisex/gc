...
class ExtraFeeTestSuite extends MatcherSuiteBase {
  ...
  Seq(
    aliceNode.transfer(aliceAcc.address, bobAcc.address, defaultAssetQuantity / 2, 0.005.GXB, Some(asset0), None, 2).id,
    aliceNode.transfer(aliceAcc.address, bobAcc.address, defaultAssetQuantity / 2, 0.009.GXB, Some(asset1), None, 2).id,
    bobNode.transfer(bobAcc.address, aliceAcc.address, defaultAssetQuantity / 2, 0.005.GXB, Some(asset2), None, 2).id
  ...
  "When matcher executes orders" - {
    "with one Smart Account and one Smart Asset" - {
      "then fee should be 0.003 + 0.004 (for Smart Asset only, not Smart Account)" in {
        ...
        matcherNode.reservedBalance(aliceAcc)("GXB") shouldBe expectedFee
