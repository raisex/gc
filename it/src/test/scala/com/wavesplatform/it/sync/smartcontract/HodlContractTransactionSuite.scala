...
class HodlContractTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("setup contract account with GXB") {
    val tx =
  ...
  test("setup caller account with GXB") {
    val tx =
      TransferTransactionV2
        .selfSigned(
          version = 2,
  ...
  test("set contract to contract account") {
    val scriptText =
      """
        ...
        |   if (isDefined(pmt.asset)) then throw("can hodl GXB only at the moment")
        |   else {
  ...
  test("caller deposits GXB") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          p = Some(ContractInvocationTransaction.Payment(1.5.GXB,None)),
          timestamp = System.currentTimeMillis(),
          fee = 1.GXB
        )
        .explicitGet()
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 1.5.GXB)
    val balanceAfter = sender.accountBalances(contract.address)._1

    (balanceAfter - balanceBefore) shouldBe 1.5.GXB
  }
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.51.GXB))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.GXB
        )
  ...
  test("caller can withdraw less than he owns") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.49.GXB))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.GXB
        )
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 0.01.GXB)
    (balanceAfter - balanceBefore) shouldBe -1.49.GXB
  }
}
