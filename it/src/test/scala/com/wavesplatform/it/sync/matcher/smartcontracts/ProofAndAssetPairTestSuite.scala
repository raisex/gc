    ...
    "positive scenarios of order placement" - {
      "set contracts with AssetPairs/all tx fields/true/one proof and then place order" in {
        for (i <- Seq(sc1, sc3, sc4, sc5)) {
          ...
          val aliceOrd1 = matcherNode
            .placeOrder(aliceAcc, predefAssetPair, OrderType.BUY, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)

          val aliceOrd2 = matcherNode
            .placeOrder(aliceAcc, aliceWavesPair, OrderType.SELL, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
      ...
      "place order and then set contract on AssetPairs/true/all fields/one proof" in {
        for (i <- Seq(sc1, sc3, sc4, sc5)) {
          log.debug(s"contract: $i")
          val aliceOrd1 = matcherNode
            .placeOrder(aliceAcc, predefAssetPair, OrderType.BUY, 500, 2.GXB * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)

          val aliceOrd2 = matcherNode
            .placeOrder(aliceAcc, aliceWavesPair, OrderType.SELL, 500, 2.GXB * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(aliceWavesPair, aliceOrd2, "Accepted", 1.minute)
          ...
          val bobOrd1 = matcherNode
            .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 500, 2.GXB * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          val bobOrd2 = matcherNode
            .placeOrder(bobAcc, aliceWavesPair, OrderType.BUY, 500, 2.GXB * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
      ...
      "place order and then set contract with many proofs" in {
        ...
        val transferTx = aliceNode.transfer(aliceAcc.address, bobAcc.address, 1000, 0.005.GXB, Some(aliceAsset), None, 2)
        ...
        for ((i, step) <- Seq(sc5, sc6).zipWithIndex) {
          ...
          val bobOrd1 = matcherNode
            .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          val bobOrd2 = matcherNode
            .placeOrder(bobAcc, aliceWavesPair, OrderType.SELL, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
    ...
    "negative scenarios of order placement" - {
      ...
      "place order and then set contract" in {
        for ((contract, i) <- Seq(sc2, sc7, sc8, sc9).zip(Seq(2, 7, 8, 9))) {
          ...
          val aliceOrd1 = matcherNode
            .placeOrder(aliceAcc, predefAssetPair, OrderType.BUY, 100, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)
          ...
          val bobOrd1 = matcherNode
            .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 100, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id

          val bobOrd2 = matcherNode
            .placeOrder(bobAcc, aliceWavesPair, OrderType.BUY, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          ...
          updatedAliceBalance shouldBe (aliceBalance - 0.014.GXB)
