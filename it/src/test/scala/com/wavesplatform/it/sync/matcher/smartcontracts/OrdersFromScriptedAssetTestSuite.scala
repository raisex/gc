...
class OrdersFromScriptedAssetTestSuite extends MatcherSuiteBase {
  ...
  "if counter order has expired then cancel its only, not a submitted order" in {
    val pair = AssetPair.createAssetPair("GXB", AllowAssetId).get

    // place expiring counter order
    val counterId =
      matcherNode
        .placeOrder(matcherPk, pair, OrderType.SELL, 2.GXB, 10, version = 2, fee = smartTradeFee, timeToLive = 61.seconds)
        .message
        .id
    matcherNode.waitOrderStatus(pair, counterId, "Accepted")
    ...
    val submittedId =
      matcherNode.placeOrder(matcherPk, pair, OrderType.BUY, 2.GXB, 10, version = 2, fee = smartTradeFee).message.id
  ...
  "can't execute against unscripted, if the script returns FALSE" in {
    ...
    val setAssetScriptId = matcherNode.setAssetScript(AllowAsset2Id, matcherPk.address, 1.GXB, Some(DenyBigAmountScript.bytes().base64)).id
  ...
  "can't execute against scripted, if one script returns FALSE" in {
    ...
    info("update a script")
    val setAssetScriptId = matcherNode.setAssetScript(AllowAsset3Id, matcherPk.address, 1.GXB, Some(DenyBigAmountScript.bytes().base64)).id
    matcherNode.waitForTransaction(setAssetScriptId)
