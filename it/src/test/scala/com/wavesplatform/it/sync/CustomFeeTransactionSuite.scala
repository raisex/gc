...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
      | GXB.fees.transfer.$assetId = 100000
      | GXB.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("GXB.miner.enable=no").withFallback(minerConfig)
