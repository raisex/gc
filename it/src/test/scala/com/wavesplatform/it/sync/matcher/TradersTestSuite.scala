...
class TradersTestSuite extends MatcherSuiteBase {
  ...
  "Verifications of tricky ordering cases" ignore {
    ...
    "owner moves assets/GXB to another account and order become an invalid" ignore {
      ...
      "order with assets" - {
        ...
        "leased GXB, insufficient fee" in {
      ...
      "order with GXB" - {
        "leased GXB, insufficient fee for one ExchangeTransaction" in {
          // Amount of waves in order is smaller than fee
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1

          val oldestOrderId = bobPlacesWaveOrder(bobWavesPair, 1, 10.GXB * Order.PriceConstant)
          val newestOrderId = bobPlacesWaveOrder(bobWavesPair, 1, 10.GXB * Order.PriceConstant)

          //      waitForOrderStatus(matcherNode, bobAssetIdRaw, id, "Accepted")
          val leaseAmount = bobBalance - exTxFee - 10.GXB - matcherFee
          val leaseId     = bobNode.lease(bobAcc.address, aliceAcc.address, leaseAmount, exTxFee, 2).id
          matcherNode.waitForTransaction(leaseId)
        ...
        "leased GXB, insufficient GXB" in {
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
          val price      = 1.GXB
