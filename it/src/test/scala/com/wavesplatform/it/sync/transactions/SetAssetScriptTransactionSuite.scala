...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough GXB") {
    val (balance, eff) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64)), "negative GXB balance")
    nodes.waitForHeightArise()
    notMiner.assertBalances(firstAddress, balance, eff)

    val leaseAmount = 1.GXB
