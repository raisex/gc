    ...
    "scenarios of order placement" - {
      ...
      "place order and then set contract on BUY type" in {
        ...
        val bobOrd1 = matcherNode
          .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 1, 10.minutes)
          .message
          .id
        val bobOrd2 = matcherNode
          .placeOrder(bobAcc, aliceWavesPair, OrderType.BUY, 500, 2.GXB * Order.PriceConstant, smartMatcherFee, version = 1, 10.minutes)
          .message
          .id
