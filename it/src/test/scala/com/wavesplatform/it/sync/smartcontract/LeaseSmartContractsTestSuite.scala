...
class LeaseSmartContractsTestSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("set contract, make leasing and cancel leasing") {
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (minFee + setScriptFee + 0.2.GXB),
                            eff1 + 9 * transferAmount - (minFee + setScriptFee + 0.2.GXB))
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (2 * minFee + setScriptFee + 2 * 0.2.GXB),
                            eff1 + 10 * transferAmount - (2 * minFee + setScriptFee + 2 * 0.2.GXB))
