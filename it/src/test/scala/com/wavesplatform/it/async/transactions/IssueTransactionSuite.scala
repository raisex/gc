...
class IssueTransactionSuite extends BaseTransactionSuite with TableDrivenPropertyChecks {
  ...
  private val assetFee = 5.GXB

  test("asset issue changes issuer's asset balance; issuer's GXB balance is decreased by fee") {
  ...
  test("Not able to create asset when insufficient funds") {
    ...
    val f = for {
      ...
      bigAssetFee = firstAddressEffectiveBalance + 1.GXB
