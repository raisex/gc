...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  private val defaultFee     = 2.GXB
  private val leasingAmount  = 5.GXB

  test("leasing GXB decreases lessor's eff.b. and increases lessee's eff.b.; lessor pays fee") {
    val f = for {
  ...
  test("can not make leasing without having enough GXB") {
    val f = for {
      ...
      leaseFailureAssertion <- assertBadRequest(sender.lease(secondAddress, firstAddress, secondBalance + 1.GXB, defaultFee))
  ...
  test("can not make leasing without having enough GXB for fee") {
    val f = for {
      fb <- traverse(nodes)(_.height).map(_.min)
  ...
  test("can not make leasing without having enough your GXB to self") {
    val f = for {
      fb <- traverse(nodes)(_.height).map(_.min)

      (firstBalance, firstEffBalance) <- notMiner.accountBalances(firstAddress)
      transferFailureAssertion        <- assertBadRequest(sender.lease(firstAddress, firstAddress, firstBalance + 1.GXB, fee = defaultFee))
      _                               <- traverse(nodes)(_.waitForHeight(fb + 2))
