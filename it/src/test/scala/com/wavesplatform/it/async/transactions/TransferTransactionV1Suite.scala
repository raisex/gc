...
class TransferTransactionV1Suite extends BaseTransactionSuite with TransferSending with CancelAfterFailure {
  ...
  private val transferAmount       = 5.GXB
  private val leasingAmount        = 5.GXB
  private val leasingFee           = 0.003.GXB
  private val transferFee          = 0.002.GXB
  private val issueFee             = 5.GXB

  test("asset transfer changes sender's and recipient's asset balance; issuer's.GXB balance is decreased by fee") {
    val f = for {
  ...
  test("GXB transfer changes GXB balances and eff.b.") {
    val f = for {
      ((firstBalance, firstEffBalance), (secondBalance, secondEffBalance)) <- notMiner
        .accountBalances(firstAddress)
        .zip(notMiner.accountBalances(secondAddress))
  ...
  test("invalid signed GXB transfer should not be in UTX or blockchain") {
    def invalidByTsTx(ts: Long) =
      TransferTransactionV1
        .selfSigned(None, sender.privateKey, AddressOrAlias.fromString(sender.address).right.get, 1, ts, None, 1.GXB, Array.emptyByteArray)
        .right
        .get
  ...
  test("can not make transfer without having enough of GXB") {
    val f = for {
      fb <- traverse(nodes)(_.height).map(_.min)
      ...
      transferFailureAssertion <- assertBadRequest(sender.transfer(secondAddress, firstAddress, secondBalance + 1.GXB, transferFee))
  ...
  test("can not make transfer without having enough of your own GXB") {
    val f = for {
      fb <- traverse(nodes)(_.height).map(_.min)
